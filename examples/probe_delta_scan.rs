...
fn main() {
    ...
    eprintln!(
        "src_mV,src_idx,src_def,cur_after,effect_mHz,target_freq,lookup_mV,lookup_idx,lookup_def,voltage_shift_mV,delta_per_mV"
    );

    for target_mv in &target_mvs {
        let target_uv = target_mv * 1000;
        // Find the point closest to this voltage
        let best = volt_to_freq
            .iter()
            .min_by_key(|&&(v, _, _)| (v as i64 - target_uv as i64).abs())
            .copied();
        let (src_volt, src_idx, src_def) = match best {
            Some(x) => x,
            None => continue,
        };
