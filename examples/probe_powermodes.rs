...
fn main() {
    ...
    *info.nvapi_version_mut() = NvVersion::with_struct::<NV_GPU_CLIENT_POWER_MODES_INFO>(1);
    let r1 = unsafe {
        nvapi::sys::api::NvAPI_GPU_ClientPowerModesGetInfo(gpu, ptr::from_mut(&mut *info))
    };
    println!("GetInfo st={r1}");
    if r1 == 0 {
        let b = unsafe { std::slice::from_raw_parts(ptr::from_ref(&*info).cast::<u8>(), 5388) };
        let nz = b[4..]
            .chunks(4)
            .enumerate()
    ...
    *ctrl.nvapi_version_mut() = NvVersion::with_struct::<NV_GPU_CLIENT_POWER_MODES_CONTROL>(1);
    let r2 = unsafe {
        nvapi::sys::api::NvAPI_GPU_ClientPowerModesGetControl(gpu, ptr::from_mut(&mut *ctrl))
    };
    println!("GetControl st={r2}");
    if r2 == 0 {
        let b = unsafe { std::slice::from_raw_parts(ptr::from_ref(&*ctrl).cast::<u8>(), 4108) };
