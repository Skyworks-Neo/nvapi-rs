...
fn main() {
    ...
    let qi = |id: u32| -> unsafe extern "C" fn(NvPhysicalGpuHandle, *mut core::ffi::c_void) -> i32 {
        unsafe {
            std::mem::transmute(nvapi::sys::nvapi_QueryInterface(id).expect("QI NULL"))
        }
    };
    ...
    let dump = |label: &str, buf: &[u8], slot_mode: bool| {
        if slot_mode {
            ...
            for slot in 0..8usize {
                ...
                if valid != 0 || value != 0 {
                    println!("  slot[{slot}]: valid={valid} value={value} ({} MHz)", value / 1000);
                }
            }
        } else {
            ...
            let nz: Vec<(usize, u32)> = (1..buf.len() / 4)
                .map(|i| (i, u32::from_le_bytes(buf[i * 4..i * 4 + 4].try_into().unwrap())))
    ...
    for (label, magic, size, slot_mode) in [
        ...
        for mode in [1u32, 2u32] {
            ...
            if slot_mode { buf[4..8].copy_from_slice(&mode.to_le_bytes()); }
            ...
            if st == 0 { dump(&format!("  {label} mode={mode}"), &buf, slot_mode); }
        }
    }
    ...
    for (label, magic, size) in [
        ...
        if st == 0 { dump(&format!("  {label}"), &buf, false); }
    }
    ...
    match g.base_boost_clocks(nvapi::BaseBoostMode::Base) {
        Ok((core, mem)) => println!("base_boost_clocks(Base):  core={}kHz ({} MHz), mem={}kHz ({} MHz)",
            core, core / 1000, mem, mem / 1000),
        Err(e) => println!("base_boost_clocks(Base): {e:?}"),
    }
    match g.base_boost_clocks(nvapi::BaseBoostMode::Boost) {
        Ok((core, mem)) => println!("base_boost_clocks(Boost): core={}kHz ({} MHz), mem={}kHz ({} MHz)",
            core, core / 1000, mem, mem / 1000),
