use {
    ...
    syn::{AttrStyle, MacroDelimiter, Meta, MetaList, punctuated::Punctuated, token::PathSep},
};
...
pub fn attrs_repr<A: Borrow<Attribute>, I: IntoIterator<Item = A>>(
    attrs: I,
) -> Result<Option<Ident>> {
    let repr = match attrs
        .into_iter()
        .find(|a| a.borrow().path().is_ident("repr"))
    {
        Some(attr) => attr,
        None => return Ok(None),
    };
    ...
    match repr.tokens.clone().into_iter().next() {
        ...
        _ => Err(Error::new_spanned(repr, "invalid repr")),
    }
}

pub fn attrs_require_repr<A: Borrow<Attribute>, I: IntoIterator<Item = A>>(
    attrs: I,
) -> Result<Ident> {
    attrs_repr(attrs)
        .transpose()
