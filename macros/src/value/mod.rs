...
pub fn derive_value_data(input: TokenStream) -> Result<TokenStream> {
    ...
    let symbol = get_attr::<NvValueSymbol, _, _>(&input.attrs).and_then(|a| {
        a.ok_or_else(|| call_error(format_args!("#[{}] required", NvValueSymbol::NAME)))
    })?;
    ...
    let symbols = input
        ...
        .iter()
        .map(|variant| {
            get_attr::<NvValueSymbol, _, _>(&variant.attrs).and_then(|a| {
                a.ok_or_else(|| call_error(format_args!("#[{}] required", NvValueSymbol::NAME)))
            })
        })
